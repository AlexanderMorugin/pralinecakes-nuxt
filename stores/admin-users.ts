...
export const useAdminUserStore = defineStore("adminUserStore", () => {
  ...
  const loadAdminUserOrders = async () => {
    const result = await useFetch("/api/users/admin/load-admin-user-orders", {
      baseURL: process.env.BASE_URL,
  ...
  const loadAdminUserComments = async () => {
    const result = await useFetch("/api/users/admin/load-admin-user-comments", {
      baseURL: process.env.BASE_URL,
  ...
  // const setAdminUser = (userData: IUser) => {
